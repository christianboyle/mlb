            ...
            <div class="flex items-center justify-between px-0 min-[450px]:px-4 py-2">
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="text-gray-500 dark:text-gray-400 flex items-center gap-1">
                  <div class="relative">
                    ${isHome ? `
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-house h-4 w-4 ${hasPromotion ? 'text-green-500 dark:text-green-400' : ''}" ${hasPromotion ? `title="${hasPromotion.name.replace(/"/g, '&quot;')}" onclick="window.showPromotionModal('${hasPromotion.name.replace(/'/g, "\\'")}', '${hasPromotion.description.replace(/'/g, "\\'")}', '${hasPromotion.url || ''}')"` : ''}>
                        <path d="M15 21v-8a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v8"></path>
                        <path d="M3 10a2 2 0 0 1 .709-1.528l7-5.999a2 2 0 0 1 2.582 0l7 5.999A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>
                      </svg>
                    ` : `
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-plane h-4 w-4 ${hasPromotion ? 'text-green-500 dark:text-green-400' : ''}" ${hasPromotion ? `title="${hasPromotion.name.replace(/"/g, '&quot;')}" onclick="window.showPromotionModal('${hasPromotion.name.replace(/'/g, "\\'")}', '${hasPromotion.description.replace(/'/g, "\\'")}', '${hasPromotion.url || ''}')"` : ''}>
